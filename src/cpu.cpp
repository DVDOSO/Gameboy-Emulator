...
{
    uint8_t instruction = read8(registers.pc);
    switch (instruction)
    {
    case 0x00:
    { // RLC B
    ...
    { // RLC [HL]
        ...
        uint8_t bit7 = (read8(hl) >> 7) & 1;
        write8(hl, (read8(hl) << 1) | bit7);
    ...
    { // RRC [HL]
        uint16_t hl = (registers.h << 8) | registers.l;
        uint8_t bit0 = read8(hl) & 1;
        write8(hl, (read8(hl) >> 1) | (bit0 << 7));
        read8(hl) == 0 ? setZFlag(true) : setZFlag(false);
        setNFlag(false);
        setHFlag(false);
        bit0 ? setCFlag(true) : setCFlag(false);
    ...
    { // RL [HL]
        uint16_t hl = (registers.h << 8) | registers.l;
        uint8_t bit7 = (1 << 7) & read8(hl);
        write8(hl, (read8(hl) << 1) | ((registers.f >> 4) & 1));
        read8(hl) == 0 ? setZFlag(true) : setZFlag(false);
        bit7 ? setCFlag(true) : setCFlag(false);
        setNFlag(false);
        setHFlag(false);
    ...
    { // RR [HL]
        uint16_t hl = (registers.h << 8) | registers.l;
        uint8_t bit0 = read8(hl) & 1;
        write8(hl, (read8(hl) >> 1) | (((registers.f >> 4) & 1) << 7));
        read8(hl) == 0 ? setZFlag(true) : setZFlag(false);
        bit0 ? setCFlag(true) : setCFlag(false);
        setNFlag(false);
        setHFlag(false);
    ...
    { // SWAP [HL]
        uint16_t hl = (registers.h << 8) | registers.l;
        write8(hl, ((read8(hl) >> 4) & 0x0F) | ((read8(hl) << 4) & 0xF0));
        read8(hl) == 0 ? setZFlag(true) : setZFlag(false);
        setNFlag(false);
        setHFlag(false);
        setCFlag(false);
    ...
    { // BIT 0, [HL]
        uint16_t hl = (registers.h << 8) | registers.l;
        (read8(hl) & 1) == 0 ? setZFlag(true) : setZFlag(false);
        setNFlag(false);
        setHFlag(true);
        m_cycles += 2;
        registers.pc++;
    ...
    { // BIT 1, [HL]
        uint16_t hl = (registers.h << 8) | registers.l;
        (read8(hl) & (1 << 1)) == 0 ? setZFlag(true) : setZFlag(false);
        setNFlag(false);
        setHFlag(true);
        m_cycles += 2;
        registers.pc++;
    ...
    { // BIT 2, [HL]
        uint16_t hl = (registers.h << 8) | registers.l;
        (read8(hl) & (1 << 2)) == 0 ? setZFlag(true) : setZFlag(false);
        setNFlag(false);
        setHFlag(true);
        m_cycles += 2;
        registers.pc++;
    ...
    { // BIT 3, [HL]
        uint16_t hl = (registers.h << 8) | registers.l;
        (read8(hl) & (1 << 3)) == 0 ? setZFlag(true) : setZFlag(false);
        setNFlag(false);
        setHFlag(true);
        m_cycles += 2;
        registers.pc++;
    ...
    { // BIT 4, [HL]
        uint16_t hl = (registers.h << 8) | registers.l;
        (read8(hl) & (1 << 4)) == 0 ? setZFlag(true) : setZFlag(false);
        setNFlag(false);
        setHFlag(true);
        m_cycles += 2;
        registers.pc++;
    ...
    { // BIT 5, [HL]
        uint16_t hl = (registers.h << 8) | registers.l;
        (read8(hl) & (1 << 5)) == 0 ? setZFlag(true) : setZFlag(false);
        setNFlag(false);
        setHFlag(true);
        m_cycles += 2;
        registers.pc++;
    ...
    { // BIT 6, [HL]
        uint16_t hl = (registers.h << 8) | registers.l;
        (read8(hl) & (1 << 6)) == 0 ? setZFlag(true) : setZFlag(false);
        setNFlag(false);
        setHFlag(true);
        m_cycles += 2;
        registers.pc++;
    ...
    { // BIT 7, [HL]
        uint16_t hl = (registers.h << 8) | registers.l;
        (read8(hl) & (1 << 7)) == 0 ? setZFlag(true) : setZFlag(false);
        setNFlag(false);
        setHFlag(true);
        m_cycles += 2;
        registers.pc++;
...
void execute_instruction()
{
    ...
    { // LD [a16], SP
        registers.pc++;
        uint16_t low = read8(registers.pc);
        registers.pc++;
        uint16_t high = read8(registers.pc);
        uint16_t dest = (high << 8) | low;
        write8(dest, registers.sp & 0xFF);
        write8(dest + 1, (registers.sp >> 8) & 0xFF);
        m_cycles += 5;
        registers.pc++;
        break;
    }
    ...
    { // LD DE, n16
        registers.pc++;
        registers.e = read8(registers.pc);
        registers.pc++;
        registers.d = read8(registers.pc);
        m_cycles += 3;
        registers.pc++;
        break;
    }
    ...
    { // JR e8
        registers.pc++;
        int8_t offset = read8(registers.pc);
        registers.pc++;
        registers.pc += offset;
        m_cycles += 3;
        break;
    ...
    { // LD HL, n16
        registers.pc++;
        registers.l = read8(registers.pc);
        registers.pc++;
        registers.h = read8(registers.pc);
        m_cycles += 3;
        registers.pc++;
        break;
    }
    ...
    { // DAA
        ...
        {
            if ((registers.f & (1 << 5)) || ((registers.a & 0xF) > 0x9))
                adjustment += 0x6;
    ...
    { // LD SP, n16
        registers.pc++;
        uint8_t p = read8(registers.pc);
        registers.pc++;
        uint8_t s = read8(registers.pc);
        uint16_t sp = (s << 8) | p;
        registers.sp = sp;
        m_cycles += 3;
        registers.pc++;
    ...
    { // INC [HL]
        uint16_t hl = (registers.h << 8) | registers.l;
        (read8(hl) & 0xF) == 0xF ? setHFlag(true) : setHFlag(false);
        write8(hl, read8(hl) + 1);
        read8(hl) == 0 ? setZFlag(true) : setZFlag(false);
        setNFlag(false);
        m_cycles += 3;
    ...
    { // LD A, n8
        registers.pc++;
        registers.a = read8(registers.pc);
        m_cycles += 2;
        registers.pc++;
        break;
    }
    ...
    case 0x76:
    { // HALT
        // Advance past HALT and idle; the main loop burns cycles until an interrupt wakes us.
        registers.pc++;
        halted = true;
        m_cycles += 1;
        break;
    ...
    { // ADD A, [HL]
        uint16_t hl = (registers.h << 8) | registers.l;
        uint16_t result = registers.a + read8(hl);
        ((registers.a & 0x0F) + (read8(hl) & 0x0F)) > 0x0F ? setHFlag(true) : setHFlag(false);
        result > 0xFF ? setCFlag(true) : setCFlag(false);
        registers.a = (uint8_t)result;
        registers.a == 0 ? setZFlag(true) : setZFlag(false);
        setNFlag(false);
    ...
    { // ADC A, [HL]
        uint16_t hl = (registers.h << 8) | registers.l;
        uint8_t carry = (registers.f & (1 << 4)) ? 1 : 0;
        uint16_t result = registers.a + read8(hl) + carry;
        ((registers.a & 0x0F) + (read8(hl) & 0x0F) + carry) > 0x0F ? setHFlag(true) : setHFlag(false);
        result > 0xFF ? setCFlag(true) : setCFlag(false);
        registers.a = (uint8_t)result;
        registers.a == 0 ? setZFlag(true) : setZFlag(false);
        setNFlag(false);
    ...
    { // SUB A, [HL]
        uint16_t hl = (registers.h << 8) | registers.l;
        uint16_t result = registers.a - read8(hl);
        (registers.a & 0x0F) < (read8(hl) & 0x0F) ? setHFlag(true) : setHFlag(false);
        registers.a < read8(hl) ? setCFlag(true) : setCFlag(false);
        registers.a = (uint8_t)result;
        registers.a == 0 ? setZFlag(true) : setZFlag(false);
        setNFlag(true);
        m_cycles += 2;
    ...
    { // SBC A, [HL]
        ...
        uint8_t carry = (registers.f & (1 << 4)) ? 1 : 0;
        uint16_t result = registers.a - read8(hl) - carry;
        (((registers.a & 0x0F) - (read8(hl) & 0x0F) - carry) & 0x10) ? setHFlag(true) : setHFlag(false);
        (registers.a < (read8(hl) + carry)) ? setCFlag(true) : setCFlag(false);
        registers.a = (uint8_t)result;
        registers.a == 0 ? setZFlag(true) : setZFlag(false);
        setNFlag(true);
        m_cycles += 2;
    ...
    { // CP A, [HL]
        uint16_t hl = (registers.h << 8) | registers.l;
        registers.a == read8(hl) ? setZFlag(true) : setZFlag(false);
        setNFlag(true);
        (registers.a & 0x0F) < (read8(hl) & 0x0F) ? setHFlag(true) : setHFlag(false);
        registers.a < read8(hl) ? setCFlag(true) : setCFlag(false);
        m_cycles += 2;
    ...
    { // RET NZ
        if (!(registers.f & (1 << 7)))
        {
            uint8_t low = read8(registers.sp);
            registers.sp++;
            uint8_t high = read8(registers.sp);
            registers.sp++;
    ...
    { // JP NZ, a16
        registers.pc++;
        uint8_t low = read8(registers.pc);
        registers.pc++;
        uint8_t high = read8(registers.pc);
        if (!(registers.f & (1 << 7)))
        {
            uint16_t dest = (high << 8) | low;
            registers.pc = dest;
    ...
    { // CALL NZ, a16
        ...
        if (!(registers.f & (1 << 7)))
        {
            registers.pc++;
            registers.sp--;
            write8(registers.sp, (registers.pc >> 8) & 0xFF);
            registers.sp--;
            write8(registers.sp, registers.pc & 0xFF);
            registers.pc = dest;
            m_cycles += 6;
        }
        else
    ...
    { // LDH [a8], A
        registers.pc++;
        uint8_t a8 = read8(registers.pc);
        uint16_t dest = 0xFF00 + a8;
        write8(dest, registers.a);
        m_cycles += 3;
        registers.pc++;
        break;
    }
...
void debug_execute()
{
    printf("Executing instruction %02x    ", read8(registers.pc));
    execute_instruction();
...
// Service pending interrupts. Wakes HALT regardless of IME, but only dispatches when IME is set.
// Priority: VBlank > STAT > Timer > Serial > Joypad (IF bit 0..4).
void handle_interrupts()
{
    ...
    halted = false; // a pending interrupt always wakes HALT

    if (!ime_flag)
        return; // awake, but not servicing (IME disabled)
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        {
            ime_flag = false;
            write8(0xFF0F, read8(0xFF0F) & ~(1 << i)); // acknowledge (clear IF bit)
            registers.sp--;
            write8(registers.sp, (registers.pc >> 8) & 0xFF);
            registers.sp--;
            write8(registers.sp, registers.pc & 0xFF);
            registers.pc = vectors[i];
            m_cycles += 5;
            return; // service one interrupt per check
        }
    }
}
...
void set_post_boot_state()
{
    ...
    write8(0xFF40, 0x91); // LCDC (LCD on, BG on)
